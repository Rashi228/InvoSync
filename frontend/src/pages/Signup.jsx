    ...
    <div className="min-h-[70vh] grid place-items-center">
      <div className="w-full max-w-md rounded-2xl ring-1 ring-white/10 bg-[#0f1530]/80 p-6">
        ...
        <div className="grid gap-3">
          <input className="border border-white/10 bg-white/5 rounded px-3 py-2" placeholder="Full name" value={name} onChange={e=>setName(e.target.value)} />
          <input className="border border-white/10 bg-white/5 rounded px-3 py-2" placeholder="Work email" value={email} onChange={e=>setEmail(e.target.value)} />
          ...
          <button className="bg-gradient-to-r from-indigo-500 to-emerald-500 text-white rounded px-4 py-2 disabled:opacity-60" onClick={submit} disabled={loading}>{loading? 'Creating...' : 'Sign up'}</button>
        </div>
        <p className="text-sm text-slate-400 mt-4">Already have an account? <Link to="/login" className="text-indigo-300 hover:text-indigo-200">Sign in</Link></p>
      </div>
    </div>
  )
}
